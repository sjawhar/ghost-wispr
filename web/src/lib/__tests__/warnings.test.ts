...
describe('warning filtering', () => {
  it('hides host microphone startup warnings from the banner', () => {
    ...
    expect(
      shouldDisplayWarning(
        'Microphone failed to start — recording and live transcription are disabled',
      ),
    ).toBe(false)
  })
  ...
  it('keeps unrelated warnings visible', () => {
    expect(
      shouldDisplayWarning('Deepgram API key not configured — live transcription is disabled'),
    ).toBe(true)
  })
